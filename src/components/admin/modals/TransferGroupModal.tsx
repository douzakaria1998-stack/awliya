...
export function TransferGroupModal({
  ...
  const filteredGroups = useMemo(() => {
    ...
    return groups.filter((g) => {
      // Don't hide archived unless needed; prioritize active
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/70 backdrop-blur-md animate-fade-in select-none">
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-[28px] shadow-2xl border border-slate-200/90 dark:border-slate-800 overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-black text-white truncate flex items-center gap-2">
                <span>{language === 'ar' ? 'نقل الطالب إلى فوج آخر' : 'Change Student Group'}</span>
              </h3>
        ...
        {isSuccess ? (
          /* Success Screen */
          <div
            className="flex flex-col items-center justify-center text-center p-8 sm:p-10 animate-fade-in overflow-y-auto"
          >
            <div className="w-20 h-20 rounded-3xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shadow-xs mb-5">
              <CheckCircle2 size={46} strokeWidth={2.2} />
            </div>

            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 font-bold text-xs px-4 py-1.5 shadow-2xs mb-3">
              <Sparkles size={14} />
            ...
            <div className="w-full max-w-md bg-slate-50 dark:bg-slate-800/80 border border-slate-200/90 dark:border-slate-700 rounded-2xl p-4 flex items-center justify-between gap-3 mb-8 shadow-2xs">
          ...
          <div className="flex flex-col flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">
            ...
            <div className="p-4 rounded-2xl bg-purple-50/70 dark:bg-purple-950/30 border border-purple-200/80 dark:border-purple-900/40 flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center font-black text-sm shadow-xs shrink-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate mt-0.5">
                    {language === 'ar' ? 'المستوى الحالي:' : 'Current Level:'} {student.currentLevel} • ID: {student.id}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end shrink-0">
                <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold uppercase tracking-wider mb-1">
                  {language === 'ar' ? 'الفوج النشط حالياً' : 'Current Active Group'}
                </span>
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-purple-100 dark:bg-purple-900/70 text-purple-800 dark:text-purple-200 text-xs font-bold border border-purple-200 dark:border-purple-800">
                  <School size={13} />
            ...
            {targetGroup && (
              <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-900/10 via-indigo-900/15 to-emerald-900/10 dark:from-purple-950/40 dark:via-indigo-950/50 dark:to-emerald-950/40 border border-purple-200/80 dark:border-purple-800/80 animate-fade-in shadow-2xs">
                <div className="flex items-center justify-between gap-3 text-xs sm:text-sm">
                  {/* From Group */}
                  <div className="flex-1 min-w-0 bg-white/80 dark:bg-slate-850 p-3 rounded-xl border border-purple-100 dark:border-slate-700">
                  ...
                  {/* To Group */}
                  <div className="flex-1 min-w-0 bg-emerald-50 dark:bg-emerald-950/50 p-3 rounded-xl border border-emerald-200 dark:border-emerald-800">
            ...
            <div className="space-y-2.5">
              ...
              <div className="relative w-full">
                <input
                  ...
                  className="w-full rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs sm:text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all placeholder:text-slate-400"
                  style={{
                    height: '44px',
              ...
              {/* Group Roster Cards */}
              <div className="max-h-56 overflow-y-auto space-y-2 rounded-2xl border border-slate-200 dark:border-slate-800 p-2 bg-slate-50/50 dark:bg-slate-850/50">
                  ...
                  filteredGroups.map((grp) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          {/* Code Pill */}
                          <span className="font-mono font-black text-xs px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 shrink-0">
                            {grp.code}
                          </span>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium flex items-center gap-2 flex-wrap mt-0.5">
                              <span>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs sm:text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all p-3"
              />
            </div>

            {/* 5. Non-Destructive Data Safety Notice */}
            <div className="rounded-2xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-900/60 p-3.5 flex items-start gap-3 text-xs text-indigo-900 dark:text-indigo-200 leading-relaxed font-medium">
            ...
            {/* Error Message */}
            {errorMsg && (
              <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800 text-xs text-rose-700 dark:text-rose-300 flex items-center gap-2 font-bold animate-shake">
        ...
        {!isSuccess && (
          <div
            className="border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-850 flex items-center justify-end gap-3 shrink-0"
            style={{ padding: '14px 24px' }}
          >
            <button
              type="button"
              onClick={handleCloseAndReset}
              className="rounded-xl px-5 h-10 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className={`rounded-xl px-6 h-10 text-xs font-black text-white flex items-center gap-2 transition-all shadow-md cursor-pointer ${
