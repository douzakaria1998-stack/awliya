...
export function AdminAttendanceScreen() {
  ...
  return (
    <div className={`w-full select-none ${isRTL ? 'text-right' : 'text-left'}`}>
      ...
      {/* 3. Compact Attendance Modal Popup */}
      {isDrawerOpen && activeDrawerGroup && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 sm:p-6 select-none">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden z-50 animate-in zoom-in-95 duration-200">
            {/* Header */}
            <div
                  ...
                  <table className={`w-full text-xs ${isRTL ? 'text-right' : 'text-left'}`}>
                    <thead className="bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200/80 dark:border-slate-800 text-slate-400 font-bold text-[11px]">
                      <tr>
                        ...
                        <th className="py-3 px-3 font-extrabold text-center">
                          <div>
                      ...
                      {drawerStudents.map((st, idx) => {
                        ...
                        return (
                          <tr key={st.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                            {/* 1. Student Name */}
                            <td
                              className="py-3 font-bold text-slate-900 dark:text-white"
                              style={{
                                paddingLeft: isRTL ? '12px' : '16px',
                                paddingRight: isRTL ? '16px' : '12px',
                              }}
