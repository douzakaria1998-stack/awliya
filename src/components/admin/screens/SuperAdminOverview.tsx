...
export function SuperAdminOverview() {
  ...
  return (
    <div className="space-y-10 sm:space-y-12">
      {/* 1. Top Welcome Banner with Generous Luxury Spacing */}
      <div className="p-8 sm:p-10 lg:p-12 rounded-[32px] bg-gradient-to-r from-slate-900 via-purple-950 to-slate-900 text-white shadow-2xl border border-purple-900/50 relative overflow-hidden">
        <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-8 sm:gap-10">
          <div className="max-w-2xl space-y-3.5">
            ...
            <p className="text-xs sm:text-sm lg:text-base text-slate-300 font-medium leading-relaxed max-w-xl">
          ...
          <div className="flex flex-wrap items-center gap-4 shrink-0">
            {pendingCount > 0 && (
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs sm:text-sm transition-all shadow-lg flex items-center gap-2.5 cursor-pointer hover:scale-105 active:scale-95"
              >
                <Clock size={18} />
            ...
            {currentRole === 'teacher' ? (
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2.5 cursor-pointer hover:scale-105 active:scale-95"
              >
                <BookOpen size={18} />
              ...
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2.5 cursor-pointer hover:scale-105 active:scale-95"
              >
                <Users size={18} />
        ...
        {/* Ambient Glow */}
        <div className="absolute -left-20 -bottom-20 w-96 h-96 rounded-full bg-purple-600/20 blur-3xl pointer-events-none" />
        <div className="absolute -right-20 -top-20 w-96 h-96 rounded-full bg-rose-600/20 blur-3xl pointer-events-none" />
      </div>

      {/* 2. Key Metrics Grid (4 Stat Cards with Generous Gaps & Internal Padding) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 sm:gap-8">
        {/* Card 1: Active Students */}
        <div className="p-7 sm:p-8 rounded-[28px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4 transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400">الطلاب النشطون</span>
          ...
          <div className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 dark:text-white font-mono tracking-tight">
            {activeStudentsCount}
          </div>
        ...
        <div className="p-7 sm:p-8 rounded-[28px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4 transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400">طلبات التسجيل المعلقة</span>
        ...
        <div className="p-7 sm:p-8 rounded-[28px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4 transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400">مسارات اللغات المعتمدة</span>
        ...
        <div className="p-7 sm:p-8 rounded-[28px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4 transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400">الشهادات الصادرة (CEFR)</span>
      ...
      {/* 3. Middle Section: Pending Approvals & Live Activity Stream with Wide Spacing */}
        ...
        <div className="lg:col-span-2 rounded-[32px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 p-8 sm:p-10 shadow-2xs space-y-6">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-2xl bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold shadow-2xs">
                <Clock size={22} />
              </div>
              <div>
          ...
          {pendingCount === 0 ? (
            <div className="text-center py-14 text-slate-400 text-sm font-semibold">
            ...
            <div className="space-y-4">
              {pendingApprovals
                ...
                .map((appr) => (
                  <div
                    key={appr.id}
                    className="p-5 sm:p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-5 transition-all hover:border-slate-300 dark:hover:border-slate-650"
                  >
                    ...
                    <div className="flex items-center gap-3 shrink-0 self-end sm:self-center">
                      <button
                        type="button"
                        onClick={() => setActiveTab('approvals')}
                        className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition-all shadow-xs cursor-pointer hover:scale-105 active:scale-95"
                      >
                        معاينة وقبول
                      </button>
                    </div>
        ...
        <div className="rounded-[32px] bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 p-8 sm:p-10 shadow-2xs flex flex-col justify-between space-y-6">
          <div className="space-y-6">
            <div className="flex items-center gap-4 pb-2 border-b border-slate-100 dark:border-slate-800">
              <div className="w-11 h-11 rounded-2xl bg-blue-500/15 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold shadow-2xs">
                <ShieldCheck size={22} />
              </div>
              <div>
                <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">سجل العمليات</h3>
            ...
            <div className="space-y-4">
              {activityLogs.slice(0, 4).map((log) => (
                <div key={log.id} className="text-xs space-y-1.5 pb-4 border-b border-slate-100 dark:border-slate-800 last:border-0 last:pb-0">
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="font-black text-slate-700 dark:text-slate-200">{log.actorName}</span>
