...
export function SuperAdminOverview() {
  ...
  return (
    <div className="space-y-8">
      {/* 1. Top Welcome Banner with Generous Spacing */}
      <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-slate-900 via-purple-950 to-slate-900 text-white shadow-xl border border-purple-900/50 relative overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-4 shrink-0">
            {pendingCount > 0 && (
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs sm:text-sm transition-all shadow-lg flex items-center gap-2.5 cursor-pointer"
              >
                <Clock size={18} />
            ...
            {currentRole === 'teacher' ? (
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2.5 cursor-pointer"
              >
                <BookOpen size={18} />
              ...
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2.5 cursor-pointer"
              >
                <Users size={18} />
      ...
      {/* 2. Key Metrics Grid (4 Stat Cards with Generous Breathing Room) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Card 1: Active Students */}
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-3.5">
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-3.5">
          <div className="flex items-center justify-between">
            ...
            <div className="w-11 h-11 rounded-2xl bg-purple-50 dark:bg-purple-950/50 text-purple-600 dark:text-purple-400 flex items-center justify-center">
              <Languages size={22} />
            </div>
          </div>
          <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 rounded-3xl bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 p-7 sm:p-8 shadow-2xs space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="space-y-3.5">
              {pendingApprovals
                ...
                .map((appr) => (
                  <div
                    key={appr.id}
                    className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500 to-rose-500 text-white font-black text-base flex items-center justify-center shrink-0 shadow-xs">
                    ...
                    <div className="flex items-center gap-2.5 shrink-0 self-end sm:self-center">
                      <button
                        type="button"
                        onClick={() => setActiveTab('approvals')}
                        className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition-all shadow-xs cursor-pointer"
                      >
                        معاينة وقبول
                      </button>
                    </div>
        ...
        <div className="rounded-3xl bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 p-7 sm:p-8 shadow-2xs flex flex-col justify-between space-y-6">
          <div className="space-y-5">
            <div className="flex items-center gap-3.5">
              ...
              <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">سجل العمليات الحديثة</h3>
            </div>
