...
export function AcademicPathScreen({ onOpenAddStudent }: AcademicPathScreenProps) {
  ...
  const getLevelName = (lvl: AcademicLevel) => {
    if (language === 'en') return lvl.nameEn || LEVEL_TITLES_EN[lvl.level]?.name || lvl.nameAr;
    if (language === 'fr') return LEVEL_TITLES_FR[lvl.level]?.name || lvl.nameAr;
    return lvl.nameAr || `المستوى ${lvl.level}`;
  };

  const getLevelStage = (lvl: AcademicLevel) => {
    if (language === 'en') return lvl.stageEn || LEVEL_TITLES_EN[lvl.level]?.stage || lvl.stageAr;
    if (language === 'fr') return LEVEL_TITLES_FR[lvl.level]?.stage || lvl.stageAr;
    return lvl.stageAr || lvl.descriptionAr || `المرحلة ${lvl.level}`;
  };
